...
class FG_eval {
     public:
      ...
      void operator()(ADvector& fg, const ADvector& vars) {
        ...
        int cont=N*3*qn+1;
        for (int i=0;i<relative_pair.size();i++)
        {//--  relative_pair[i].at(1)是qi，at(0)是k,就是对N循环的,at(2)是j
...
public:
...

    {
        ...
        for (i=0;i<qn;i++){
            for(j=0;j<M;j++){
                ...
                //--计算速度，设定在0.1范围内为0，0.1-1.1为最大速度的0.7倍，其余为最大速度
                    if (dis<0.1)
                    {
                      plan[i][5*j+k][3]=0;
                    }
        ...
        for (int qi=0; qi<qn; qi++)
        {
          for (int next=5;next<N;next+=5){
            if ((plan[qi][next][1]==plan[qi][0][1]) &&(plan[qi][next][0]==plan[qi][0][0]))
              ...
              plan[qi][0][2]=atan2(plan[qi][next][1]-plan[qi][0][1],plan[qi][next][0]-plan[qi][0][0]); //--计算角度
              break;
            }
        }
      }
      ...
      for (k=0;k<N;k++){
        for(i=0;i<qn;i++){
          for (j=i+1;j<qn;j++)
          {
            ...
            if (dis<1.3) //--当距离是1.14的时候，才放入，是这个距离才有危险么
    ...
    void Solve() {
      ...
      size_t n_constraints = N * 3 * qn; //--这应该只有等式的约束，起点和终点必须过的约束，以及需要满足动力学约束
      //size_t all_constraints = n_constraints + qn*(qn-1)*int(N/2)/2;
      size_t all_constraints = n_constraints + relative_pair.size(); //--剩下的是相对轨迹的约束
...
//TODO
      for (int qi=0;qi<qn;qi++){
        ...
        for (int i = 0; i < N; i++) {
          ...
          //--SFC数据结构：[qi]是指的第qi个agent，[count]是第几个框(总的第几个）
          //-- first是box，分别是x_min,y_min,z_min,x_max..，second是在qi下的第几个框
          //--  SFC[qi].emplace_back(std::make_pair(box, i+1));
          if ( i>1 && i==SFC[qi][count+1].second && count<SFC[qi].size()-1){
            count++;
          } //--？？？？
          //--如果当前轨迹距离安全边界框>2，那约束就为(x-2,x+2)，反之，如果<2，那就以边界为约束
      ...
      } //--等式约束，即必须经过这些点，包括的是每个机器人的起点，以及动力学约束的点

      double inter_collision=2*mission.quad_size[0]+0.2;  //-- 交互碰撞，2*radius+0.2 = 0.5
